		...
		if (isHost) {
			...
			while (server.messages.try_dequeue(new_message)){//!server.messages.empty()) {
				...
				case TcpMsgType::CONNECTION_REQUEST:
					...
					playersInfos.Add(new_message.playerID, new_message.playerName, new_message.playerID, /*NO!*/newEntity);
					server.setPlayerEntityID(new_message.playerID, /*NO!*/newEntity);
					break;
			...
			while (client.receivedMessages.try_dequeue(new_message)){
				...
				case TcpMsgType::NEW_CONNECTION:
					...
					playersInfos.Add(new_message.playerID, new_message.playerName, new_message.playerID, new_message.entityIDs.at(0));
					break;
